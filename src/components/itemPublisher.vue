...
const handleDeleted = (e) => {
   axios.delete(import.meta.env.VITE_SERVER_URL + `${'api/publishers'}/${props.item._id}`)
    .then(res => {
        alert("Đã xóa!")
...
<template>
       <li class="relative flex justify-between p-4 hover:bg-blue-100 hover:cursor-pointer dark:bg-gray-800 dark:hover:bg-gray-700">
                <router-link :to="`${props.item.email? '/borrowbook' : '/publisher'}/${props.item._id}`">
                    <div class="flex items-center space-x-4 rtl:space-x-reverse">
                        ...
                        <div class="flex-1 min-w-0">
                            <p class="text-sm font-medium text-gray-900 truncate dark:text-white">
                                {{ props.item.email? props.item.email: props.item.name }}
                            </p>
                            <p class="text-sm text-gray-500 truncate dark:text-gray-400">
                               {{ props.item.title? props.item.title: props.item.address }}
                            </p>
                        </div>
